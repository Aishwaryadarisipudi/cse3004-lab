...
static void NumberOfSegments(ArrayList<int[]> segments,int[] points,
int s, int p){
...
count += seg.get(seg.size() - 1)[1]; seg.remove(seg.size()
- 1);
}
...
seg.add(new int[]{2, 3}); seg.add(new int[]{0, 5}); seg.add(new
int[]{7, 10});
int[] point = {1, 6, 11};
int s = seg.size();
int p = point.length;
